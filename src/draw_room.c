...
/*   Updated: 2020/09/07 12:36:49 by jthuy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	def_raylen(t_map *map, t_player *player, t_drawer *drawer)
{
	drawer->ray_angle = ((drawer->cursor_x / (double)WIDTH) * player->fov)
		+ (player->angle - player->fov / (double)2);
...
double	calc_raylen(t_player *player, t_drawer *drawer, char index)
{
	...
	ray_len = sqrt(pow(drawer->barrier_f[index][0] - player->pos_x, 2) +
		pow(drawer->barrier_f[index][1] - player->pos_y, 2));
	return (ray_len);
}
